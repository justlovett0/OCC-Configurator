/*
 * usb_descriptors.c - Dual-mode USB Descriptors for Retro Gamepad Controller
 *
 * Normal mode:  XInput Gamepad (Xbox 360 controller, subtype 0x01)
...
uint16_t const* tud_descriptor_string_cb(uint8_t index, uint16_t langid) {
    ...
    switch (index) {
        ...
        case 2: {
            if (g_config_mode) {
                return _make_string_desc("Retro Gamepad Config");
            }
            if (g_device_name && g_device_name[0] != '\0') {
                return _make_string_desc(g_device_name);
            }
            return _make_string_desc("Retro Gamepad");
        }
        case 3: {
