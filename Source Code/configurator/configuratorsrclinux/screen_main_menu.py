...
class MainMenu:
                ...
                try:
                    ...
                    if config_devices:
                        ...
                        self._ctrl_status.config(
                            text=f"{first_label} detected in play mode  ({count} device{'s' if count > 1 else ''})",
                            fg=ACCENT_GREEN)
                        ...
                        self._ctrl_icon.config(text="●", fg=ACCENT_GREEN)
                        self._ctrl_status.config(
                            text=f"Dongle detected in play mode  ({count} device{'s' if count > 1 else ''})",
                            fg=ACCENT_GREEN)
    ...
    def _refresh_firmware_status(self):
        ...
        if state == "bootsel":
            ...
            self._fw_icon.config(text="●", fg=ACCENT_GREEN)
            self._fw_status.config(
                text=f"{value} detected in play mode  ({xinput_count} device{'s' if xinput_count > 1 else ''})",
                fg=ACCENT_GREEN)
            if uf2:
                bundled_date_str = get_bundled_fw_date_str(uf2)
                self._fw_detail.config(
    ...
    def _refresh_reset_card(self):
        ...
        if drive:
            ...
            self._rst_icon.config(text="●", fg=ACCENT_GREEN)
            self._rst_status.config(
                text=f"{xinput_label} detected in play mode  ({count} device{'s' if count > 1 else ''})",
                fg=ACCENT_GREEN)
            if resetFW:
                self._rst_detail.config(
            ...
            self._rst_icon.config(text="●", fg=ACCENT_GREEN)
            self._rst_status.config(
                text=f"Dongle detected in play mode  ({dongle_count} device{'s' if dongle_count > 1 else ''})",
                fg=ACCENT_GREEN)
            self._rst_detail.config(
    ...
    def _find_alternatefw_device(self):
        ...
        try:
            # Linux firmware switching relies on PyUSB + a libusb backend.
            import usb.core
                ...
                if via == 'xinput':
                    if not XINPUT_AVAILABLE:
                        self.root.after(0, lambda: messagebox.showerror(
                            "Check for Updates",
                            "Play-mode controller signaling is not available on this system."))
                        finish()
                        return
