/*
 * guitar_config.c - Flash-backed configuration storage
 *
 * v6:  Added analog sensitivity (min/max) for tilt and whammy.
 * v7:  Added LED loop (color rotation) fields.
 * v13: Added wireless_default_mode for combined Dongle/BT firmware.
 */
...
void config_set_defaults(guitar_config_t *config) {
    ...
    config->i2c_model     = I2C_MODEL_ADXL345;
    ...
    // Dongle sync button — default GPIO 15
    config->sync_pin = 15;
