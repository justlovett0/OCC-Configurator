...
static struct {
    ...
    bool    auth_seen;         // set when 360 security challenge (req 0x81) arrives
...
static bool xinput_control_xfer_cb(uint8_t rhport, uint8_t stage, tusb_control_request_t const *request) {
    ...
    case 0x81: // Get identification — 360 starts auth here
        _xinput.auth_seen = true;  // real XInput host confirmed
...
// Newer TinyUSB versions route all vendor control requests through this
// application callback before consulting custom class drivers.
