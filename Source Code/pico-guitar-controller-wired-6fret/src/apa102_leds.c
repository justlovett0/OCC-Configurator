...
static uint32_t wave_prev_mask = 0xFFFFFFFFu; // init high: avoids spurious trigger on first frame
...
void apa102_update_from_inputs(const led_config_t *cfg, uint32_t pressed_mask) {
    ...
    if (cfg->wave_enabled) {
        uint32_t trigger = pressed_mask & ~((1u << LED_INPUT_TILT) | (1u << LED_INPUT_WHAMMY));
        uint32_t rising = trigger & ~wave_prev_mask;
