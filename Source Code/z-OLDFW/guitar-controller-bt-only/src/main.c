/*
 * main.c - Pico W Dual-Mode Guitar Controller Firmware
 *
 * Supports BOTH wired USB XInput AND wireless Bluetooth Classic HID.
 * The mode is auto-detected at boot:
 *
 * Boot flow:
 ...
 *      - If timeout expires with no host → Bluetooth HID play mode
 *        (wireless gamepad, discoverable with custom device name)
 *
 * This means:
 *   - Plug in USB → wired controller, configurator detects it normally
 *   - Power from battery → waits briefly, then goes wireless Bluetooth
 *   - Magic vibration sequence (USB) → config mode reboot (same as before)
 *   - SELECT held at boot → config mode (fallback for BT-only scenario)
 */
...
#define BT_REPORT_INTERVAL_US  4000       // ~250 Hz for Bluetooth
#define LED_UPDATE_INTERVAL_US 16667      // ~60 Hz LED refresh
#define ADC_RESOLUTION         4095
#define DIGITAL_AXIS_ON        32767

// Time to wait for a USB host to enumerate before falling back to Bluetooth.
// 3 seconds is enough for any USB host to enumerate, but short enough that
// battery-powered wireless startup feels responsive.
...
//
// TinyUSB calls tud_mount_cb() when a USB host successfully enumerates
// the device. We use this flag to detect whether we're plugged into a
// real USB host vs just powered from a dumb charger/battery.
//--------------------------------------------------------------------

static volatile bool g_usb_mounted = false;

static void picow_led_blink_usb_waiting(void) {
    // Fast blink while waiting for USB host enumeration
    static uint32_t last_toggle = 0;
    static bool state = false;
    uint32_t now = to_ms_since_boot(get_absolute_time());
...
//--------------------------------------------------------------------
// Shared input reading — fills button/axis values used by both
// USB XInput and Bluetooth HID report builders
//--------------------------------------------------------------------

typedef struct {
    uint16_t buttons;
    uint16_t pressed_mask;    // For LED mapping
...
static void read_all_inputs(input_state_t *state) {
    ...
    // ── 5-pin Joystick ──
    {
        ...
        // Update buttons with joystick dpad/guide additions
        state->buttons = buttons;
    }
...
//--------------------------------------------------------------------
// USB XInput report builder (for wired mode — identical to original)
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
// Bluetooth HID report builder (for wireless mode)
//--------------------------------------------------------------------

static void build_bt_report(bt_hid_report_t *report) {
    input_state_t inp;
    read_all_inputs(&inp);

    memset(report, 0, sizeof(bt_hid_report_t));
    report->report_id      = 0x01;  // Required for BLE HIDS — stripped before sending
    report->buttons        = inp.buttons;
    report->right_stick_x  = inp.right_stick_x;
    report->right_stick_y  = inp.right_stick_y;
}
...
//--------------------------------------------------------------------
// Check if SELECT button is held at boot (BT config mode fallback)
//--------------------------------------------------------------------
...
int main(void) {
    ...
    // Initialize CYW43 early — needed for the onboard LED on Pico W
    // and for Bluetooth in wireless play mode
    if (cyw43_arch_init()) {
        // CYW43 init failed — we can still do USB modes
    } else {
    ...
    // ── Step 1: Check for config mode via watchdog scratch register ──
    bool enter_config = check_scratch_config_mode();

    if (enter_config) {
        // ═══════════════════════════════════════════════════════════════
        // CONFIG MODE — USB CDC serial
        // Same as original wired firmware. Configurator works as before.
        // ═══════════════════════════════════════════════════════════════
        g_config_mode = true;
        tusb_init();
    ...
    // ── Step 2: Not config mode — try USB XInput first ──
    // Always start TinyUSB and give a real USB host time to enumerate.
    // If a host mounts us within the timeout, we stay in USB XInput mode.
    // If not (battery power, dumb charger, no cable), switch to Bluetooth.
    g_config_mode = false;
    tusb_init();

    {
        ...
        while (!g_usb_mounted) {
            ...
            if (elapsed >= USB_MOUNT_TIMEOUT_MS) {
                break;  // No USB host responded — fall through to Bluetooth
            }

            // Also check if SELECT is held during the wait — enter config mode
            // (This is the fallback for users who need config mode without the
            // magic vibration sequence, e.g. first-time Bluetooth setup.)
            // Only check once near the end to avoid false triggers.
            if (elapsed >= (USB_MOUNT_TIMEOUT_MS - 500)) {
                if (check_select_held_at_boot()) {
                    // Enter config mode (USB CDC)
                    g_config_mode = true;
                    // tusb is already initialized, just switch descriptor mode
                    // and reboot cleanly into config mode
    ...
    // ── Step 3: We now know which mode to use ──
    // Initialize shared play-mode hardware (LEDs, GPIO, ADC, I2C, debounce)
    ...
    if (g_usb_mounted) {
        ...
        while (true) {
            ...
            // Magic vibration sequence → reboot into config mode
        ...
        // ═══════════════════════════════════════════════════════════════
        // BLUETOOTH HID PLAY MODE — Wireless controller
        // No USB host responded within timeout.
        // Becomes discoverable with custom device name.
        // ═══════════════════════════════════════════════════════════════
        ...
        while (true) {
            ...
            if (now_us - last_report_us >= BT_REPORT_INTERVAL_US) {
                last_report_us = now_us;
                if (bt_hid_connected()) {
                    build_bt_report(&report);
                    bt_hid_send_report(&report);
                }
            }
